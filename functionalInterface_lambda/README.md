...
자주 변경되는 요구사항에 효과적으로 대응하기 위해 메서드의 동작을 파라미터화하는 기법을 동작 파라미터화라고 합니다.

메서드 참조나 람다 표현식을 사용하여 함수형 인터페이스를 인스턴스를 생성하고 메서드의 파라미터로 전달함으로써, 메서드의 동작을 외부에서 정의하고 변경할 수 있습니다.

## 개념

- 추상 메서드를 딱 하나만 가지고 있는 인터페이스를 함수형 인터페이스라고 합니다. (인터페이스에서는 abstract 생략 가능)
- 만약 추상 메서드가 두개 있으면 함수형 인터페이스가 아닙니다. 디폴트 메서드는 여러개 있어도 상관없습니다.
- 함수 디스크립터(Function Descriptor)라는 용어는 함수형 인터페이스의 추상 메서드가 어떤 형태인지를 설명하는 것을 의미합니다. 
예를 들어, Runnable 인터페이스의 경우에는 Runnable의 함수 디스크립터는 () -> void입니다 라고 말할 수 있습니다.
